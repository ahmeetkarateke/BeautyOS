...
import { WorkingHoursPicker, parseWorkingHours, DEFAULT_WORKING_HOURS, type WorkingHoursMap } from '@/components/working-hours-picker'
...
function SalonSettingsForm({ tenantSlug }: { tenantSlug: string }) {
  ...
  useEffect(() => {
    if (data) {
      setWorkingHours(parseWorkingHours(data.settings?.workingHours))
      reset({
        name: data.name,
  ...
  const mutation = useMutation({
    mutationFn: (values: SalonFormValues) =>
      apiFetch(`/api/v1/tenants/${tenantSlug}/settings`, {
        method: 'PATCH',
        body: JSON.stringify({ ...values, workingHours }),
      }),
